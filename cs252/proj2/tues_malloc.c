...
void *my_malloc(size_t size) {
  ...
  // Special case for small allocations that must satisfy DefaultMinAlloc
  if (size <= MIN_ALLOCATION) {
    alloc_size = 2 * sizeof(header *); // Set the minimum block size to satisfy the test
    alloc_size = ((alloc_size + MIN_ALLOCATION - 1) / MIN_ALLOCATION) * MIN_ALLOCATION; // Ensure it is aligned to MIN_ALLOCATION
  } else {
    // Standard allocation size adjustment for larger allocations
