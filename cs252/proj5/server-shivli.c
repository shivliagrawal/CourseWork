...
void handle(socket_t *sock) {
    ...
    if (strstr(buf, "\r\n\r\n") == NULL) {
        ...
        if (response_str) {
            ...
            socket_write(sock, response_str, strlen(response_str));
            free(response_str);
        }
    ...
    if (response_str) {
        ...
        for (int i = 0; i < response->num_headers; i++) {
            if (strcasecmp(response->headers[i].key, "Content-Length") == 0) {
                content_length = strtoul(response->headers[i].value, NULL, 10);
                break;
            }
        }

        // Find the end of headers ("\r\n\r\n")
        char *headers_end = strstr(response_str, "\r\n\r\n");
        size_t headers_length = headers_end ? (headers_end - response_str) + 4 : strlen(response_str);

        // Calculate total length to send
        size_t total_length = headers_length + content_length;

        // Allocate a buffer for the full response
        ...
        if (!full_response) {
            ...
            char *error_str = response_string(&response_500);
            if (error_str) {
                socket_write(sock, error_str, strlen(error_str));
                free(error_str);
            }
        ...
        // Copy headers to full_response
        memcpy(full_response, response_str, headers_length);

        // Copy body to full_response
        if (content_length > 0 && response->message_body) {
            memcpy(full_response + headers_length, response->message_body, content_length);
        }

        // Send the full response
        socket_write(sock, full_response, total_length);
