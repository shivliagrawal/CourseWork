...
void command_epsv(char *params)
{
    ...
    { 
	    ...
	    control_printf(SL_FAILURE, "500-Error: Unable to bind data socket.\r\n425 %s",
		           strerror(errno));
	   return;
        }   /* end of trying to bind */
    }   /* end of there is no specific port range we need to use */
    ...
    /* try to bind to a specific port range */
        ...
        for (;;) {
            ...
            if (bind(pasvsock, (struct sockaddr *) &sa, sizeof(sa)) == 0) {
                success = 1;
#ifdef DEBUG
                bftpd_log("Extended passive mode: Successfully bound port %d\n", port);
#endif
                break;
            }
        }   /* end of for loop */
...
void command_mput(char *filenames)
{
   ...
   from_index = 0;     /* start at beginning of filenames */
...
void do_stor(char *filename, int flags)
{
    ...
    */
	...
	if (!((i = recv(sock, buffer2, my_buffer_size - 1, 0))))
            break;
    else strncpy(buffer, buffer2, i);
        bytes_recvd += i;
...
void command_retr(char *filename)
{
	...
	mapped = bftpd_cwd_mappath(filename);
        if (! mapped)
        {
           ...
           control_printf(SL_FAILURE, "553 An unknown error occurred on the server.", 9);
           return;
        }
	...
	switch (whattodo) {
		...
		case DO_GZONLY:
            ...
            if (! buffer)
            {
               ...
               control_printf(SL_FAILURE, "553 An unknown error occurred on the server.", 0);
...
void command_chmod(char *params)
{
	...
	if (!strchr(params, ' ')) {
        ...
        if (! my_string)
        {
            control_printf(SL_FAILURE, "550: An error occurred on the server trying to CHMOD.");
            return;
        }
...
returns TRUE, if an error occurs, it returns FALSE.
*/
